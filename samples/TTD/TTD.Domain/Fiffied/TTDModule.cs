    ...
    public class TTDModule : Module
    {
        ...
        public static Module Initialize(IAdvancedEventStore store, Func<IEvent[], Task> pub)
            ...
            .Command<ReadyTransport>(
                ...
                cmd => ApplicationService.ExecuteAsync(store, cmd, "all", () => new[] { new TransportReady(cmd.TransportId, cmd.Kind, cmd.Location) }, pub))
            ...
            .Policy(Policy.On<Arrived>(e => Policy.Issue(e, () => GameEngine.When(e, store.GetTransports("all").GetAwaiter().GetResult()))))
